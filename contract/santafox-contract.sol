...
abstract contract SANTAFOXBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
   // MAIN TOKEN PROPERTIES
   string private constant NAME = "SantaFox";
   string private constant SYMBOL = "SANTAFOX";
   uint8 private constant DECIMALS = 9;
   uint256 private constant _totalTokens = 100_000_000 * 10**DECIMALS; //total supply
 
  // marketing and dev address
   address private _marketingWallet = 0x6adE82216b80978B88257620F6B6D4696f075e52;
   address private _devAddress = 0x2f922F1D3f1C8188AC256096b20Ef01F15CA9f2B;
   ...
   uint8 public _poolFee; //The total fee to be taken and added to the pool, this includes all fees
   ...
   // FEES & REWARDS
   bool private _isRewardSwapEnabled; // True if the contract should swap for liquidity & reward pool, false otherwise
   ...
   // HOLD LIMIT
   bool private _isTokenHoldEnabled; //True if a wallet should not hold more than specified amount
   uint256 private _maxHoldAmount;
   uint8 private _maxHoldPercentage = 1;
   ...
   constructor (address routerAddress) {
       ...
       // Initialize PancakeSwap V2 router and SANTAFOX <-> BNB pair.
       setPancakeSwapRouter(routerAddress);
 
       _maxHoldAmount = 1_200_000 * 10**DECIMALS; //cannot hold more than 1.2% of total supply in a single wallet
 
       // 1% liquidity fee, 2% reward fee, 6% marketing fee, 4% dev fee
       setFees(1, 2, 6, 4);
       _highBuyFee = 49;
   ...
   function activate() public onlyOwner {
       ...
       setTransactionSellLimit(400_000 * 10**DECIMALS); //cannot sell more than 0.40% of Total Supply at once
       setTransactionBuyLimit(900_000 * 10**DECIMALS); //cannot buy more than 0.90% of Total Supply at once
       activateBuying(true);
       onActivated();
   }
   ...
   function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
       ...
       //Perform reward cycle calculations and reward destribution if required
       ...
       // Add the sender/recipient to reward queue
   ...
   // Trigger the rewards manually when volume is too low
   function executeRewardsManually(uint256 amount) public onlyOwner{
       executeSwap(amount);
   }
   ...
   // This function swaps a {tokenAmount} of SANTAFOX tokens for BNB and returns the total amount of BNB received
   function swapTokensForBNB(uint256 tokenAmount) public returns(uint256) {
       uint256 initialBalance = address(this).balance;
      
       // Generate pair for SANTAFOX -> WBNB
   ...
   function swapBNBForTokens(uint256 bnbAmount, address to) internal returns(bool) {
       // Generate pair for WBNB -> SANTAFOX
       address[] memory path = new address[](2);
   ...
   // Returns how many more $SANTAFOX tokens are needed in the contract before triggering a swap
   ...
   function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 marketingFee, uint8 devFee) public onlyOwner {
       ...
       require(rewardFee >= 0 && rewardFee <= 5, "Reward fee must be between 0% and 5%");
       require(marketingFee >= 0 && marketingFee <= 10, "Marketing fee must be between 0% and 10%");
       require(devFee >= 0 && devFee <= 5, "Dev fee must be between 0% and 5%");
   ...
   function isSwapEnabled() public view returns (bool) {
       return _isRewardSwapEnabled;
   }
 
 
   function setSwapEnabled(bool isEnabled) public onlyOwner {
       _isRewardSwapEnabled = isEnabled;
   }
...
contract SANTAFOX is SANTAFOXBase {
   ...
   uint256 private _totalBNBAsSANTAFOXClaimed; // The total number of BNB that was converted to SANTAFOX and claimed by all addresses
   mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
   mapping(address => uint256) private _bnbAsSANTAFOXClaimed; // The amount of BNB converted to SANTAFOX and claimed by each address
   ...
   mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SANTAFOX automatically
   ...
   bool private _reimburseAfterSANTAFOXClaimFailure; // If true, and SANTAFOX reward claim portion fails, the portion will be given as BNB instead
   ...
   constructor (address routerAddress) SANTAFOXBase(routerAddress) {
   ...
   function onActivated() internal override {
       super.onActivated();
       setRewardAsTokensEnabled(false);
       setAutoClaimEnabled(true);
       setReimburseAfterSANTAFOXClaimFailure(true);
   ...
   function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
       ...
       bool isSelling = isPancakeSwapPair(recipient);
       if (!isSelling) {
           // Wait for a dip, SANTAFOX diamond hands
           return;
       }
   ...
   function claimReward(address user) public {
       ...
       require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SANTAFOX balance");
   ...
   function doClaimReward(address user) private returns (bool) {
       ...
       // Claim SANTAFOX tokens
       if (!claimSANTAFOX(user, claimBnbAsTokens)) {
           // If token claim fails for any reason, award whole portion as BNB
           if (_reimburseAfterSANTAFOXClaimFailure) {
   ...
   function claimSANTAFOX(address user, uint256 bnbAmount) private returns (bool) {
       if (bnbAmount == 0) {
           return true;
       }
       ...
       _bnbAsSANTAFOXClaimed[user] += bnbAmount;
       _totalBNBAsSANTAFOXClaimed += bnbAmount;
       return true;
   }
   ...
   function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
       ...
       if (balance == 0) {
           // Receiving $SANTAFOX on a zero balance address:
       ...
       // Depending on the % of $SANTAFOX tokens transferred, relative to the balance, we might need to extend the period
   ...
   function bnbRewardClaimedAsSANTAFOX(address byAddress) public view returns (uint256) {
       return _bnbAsSANTAFOXClaimed[byAddress];
   }
   ...
   function totalBNBClaimedAsSANTAFOX() public view returns (uint256) {
       return _totalBNBAsSANTAFOXClaimed;
   }
   ...
   // Will be used to exclude pcs fees/token vesting addresses from rewards
   ...
   function reimburseAfterSANTAFOXClaimFailure() public view returns(bool) {
       return _reimburseAfterSANTAFOXClaimFailure;
   }
 
 
   function setReimburseAfterSANTAFOXClaimFailure(bool value) public onlyOwner {
       _reimburseAfterSANTAFOXClaimFailure = value;
   }
   ...
   //Rescue stucked BEP20 tokens
   function rescueBEP20(IERC20 token, address to, uint8 percent) public onlyOwner{
       ...
       emit Transfer(msg.sender, to, balanceToWithdraw);
   }   
